...
var f_slova: text;
    ...
    i, pocetRiadkov, pocetChyb, cislo: integer;
    riadok, hladaneSlovo, zadaneSlovo, slovo: string;
    ch: char;
    jeVyskyt: boolean;
...
function poslednePismena(slovo: string): string;
begin
  poslednePismena := copy(slovo, length(slovo) - 2, 3);
end;

procedure farebnePoslednePismena(slovo: string);
begin
  write(copy(slovo, 1, length(slovo) - 3));

  textbackground(4);
  write(copy(slovo, length(slovo) - 2, 3));
  textbackground(0);
